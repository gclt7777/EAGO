function Offspring = Operator(Problem,Particles,Loser,Winner,phi)
...
[Gbest,CrowdDis] = subUpdateGbest(Particles,N);
ParentPool = TournamentSelection(2,N,-CrowdDis);
GbestDec   = Gbest(ParentPool).decs;
%% Competitive swarm optimizer
r1     = repmat(rand(N,1),1,D);
r2     = repmat(rand(N,1),1,D);
r3     = repmat(rand(N,1),1,D);
OffVel = r1.*LoserVel + r2.*(WinnerDec-LoserDec) + r3.*phi.*(GbestDec-LoserDec);
