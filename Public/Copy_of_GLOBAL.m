...
%--------------------------------------------------------------------------
    ...
    properties(SetAccess = ?PROBLEM)
        ...
        %evaluation = 6800000;
        %evaluation = 1000000;             % Maximum number of evaluations
        %evaluation = 17000000;             % Maximum number of evaluations
    end
    ...
    methods
        ...
        %       P = obj.Initialization()
            ...
            ND1    = NDSort(Population.objs,1) == 1;
	        Population_2 = Population(ND1);
            Metrics = {@IGD};
            Score     = cellfun(@(S)GLOBAL.Metric(S,Population_2,obj.PF),Metrics,'UniformOutput',false);
            fid = fopen('F:\624\journal\picture\EAGO_DTLZ2_500.txt', 'A'); 
            obj.runtime = obj.runtime + toc;
            tic;
			fprintf(fid,'%.4f %d %d\r\n',Score{:},obj.evaluated,obj.runtime);
			fclose(fid); 
        end
    ...
    methods(Access = private, Static)
        ...
        function Output(obj)
            fprintf('%s on %s, %d objectives %d variables, run %d (%6.2f%%), %.2fs passed...\n',...
             ...
             Population_2   = obj.result{end}(Feasible(NonDominated));
             Metrics = {@IGD};
             Score     = cellfun(@(S)GLOBAL.Metric(S,Population_2,obj.PF),Metrics,'UniformOutput',false);
             fid = fopen('F:\624\journal\picture\EAGO_DTLZ2_500.txt', 'A'); 
			 fprintf(fid,'%.4f %d %d\r\n',Score{:},obj.evaluated,obj.runtime);
            ...
            if obj.evaluated >= obj.evaluation
                if 1
                    ...
                    fid = fopen('F:\624\journal\picture\EAGO_DTLZ2_500_population.txt', 'wt'); 
                    fprintf(fid,[repmat('%f\t', 1, size(Population.objs,2)), '\n'], Population.objs');
                    fclose(fid);
